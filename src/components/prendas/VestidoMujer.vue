    ...
    <div class="container">
        ...
        <div class="comands">
            <section class="buttons">
                ...
                <button class="btn" @click="changeSize(2)">Talla Pequeña</button>
                <button class="btn" @click="changeSize(0)">Resetear</button>
                <button class="btn" @click="changeSize(3)">Talla XXL</button>
            </section>

            <section class="color">
                <h3>Escoja el color del suéter</h3>
                ...
                <section class="buttons_colors">
                    ...
                    <button class="btn_color" @click="checkClothPart(5, $event)">Prenda completa</button>
                </section>
            </section>
        </div>
...
const init = () => {
    ...
    loader.load(
        "/design/dress_modern_t-_shirt___green_woman.glb",
        (gltf) => {
            model = gltf.scene;
            ...
            model.traverse((object) => {
                ...
                if (object.isMesh) {
                    ...
                    model.traverse((child) => {
                        if (child.name) {
                            console.log('Nombre:', child.name);
                        }
                    });
                    // object.material.color.set(0xE4080A);
                }
            });
        },
        undefined,
...
const changeColor = (sueter, parte) => {
    ...
    switch (parte) {
        case "torso":
            ClothePart = sueter.getObjectByName(
                "weld_chompa_varon_3b_Material719171_0"
            );
            break;
        case "cuello":
            ClothePart = sueter.getObjectByName(
                "weld_chompa_varon_3b_FABRIC_8_FRONT_720664_0"
            );
            break;
        case "ruedo":
            ClothePart = sueter.getObjectByName(
                "weld_chompa_varon_3b_FABRIC_6_FRONT_719499_0"
            );
            break;
        case "manga":
            ClothePart = sueter.getObjectByName(
                "weld_chompa_varon_3b_FABRIC_7_FRONT_719593_0"
            );
            break;
        case "Sketchfab_model":
            ClothePart = sueter.getObjectByName(
                "Sketchfab_model"
            );
            break;
...
function CallColorChange(sueter) {
    watch(
        [clothesPart, clothColor],
        ([newPart, newColor], [oldPart, oldColor]) => {
            if (newColor != oldColor) {
                if (newPart == 1) {
                    ...
                    changeColor(sueter, "Sketchfab_model");
                }
            }
        }
    );
